...
const experiences = [
  {
    ...
    bullets: [
      'Google Apps Script로 기업 매출·거래액 데이터 자동 수집 파이프라인 구축<br>→ 타겟 발굴 시간 <strong>80% 단축</strong>, 맞춤형 콜드메일 <strong>11,674건</strong> 발송·평균 열람률 <strong>64.5%</strong> 달성',
      '회신율 <strong>1.7% 벽</strong>에 부딪혔을 때, 발송량이 아닌 개인화가 답이라 판단<br>— Perplexity API + Gemini AI로 기업별 최신 뉴스를 분석해 맞춤 서론을 자동 생성하는 <strong>AI 챗봇 직접 개발</strong><br>→ 학술 SSR 지표 적용 A/B 테스트로 <strong>회신율 3.8% (2.2배↑)</strong> 달성',
      '박람회 명함 수집 → 콜드비짓 → 즉시 팔로업까지 연결되는 <strong>다채널 아웃바운드 루틴</strong> 정립<br>(열람 추적 기반 최적 타이밍 자동 알림 포함)<br>→ 온라인 거래액 1억 이상 우량 기업 <strong>30곳 미팅 성사 → 5개사 온보딩 전환</strong>',
      '자동화 대시보드·AI 챗봇을 팀 전체가 활용할 수 있도록 배포<br>→ <strong>SDR 팀 전체 아웃바운드 역량 표준화</strong>에 기여',
    ],
    kpis: [
  ...
  },
  {
    ...
    bullets: [
      'Try Everything 박람회 부스 직접 운영<br>— 현장에서 <strong>리드 수집 및 즉석 데모</strong> 진행, AI 음성 합성 기술의 가치를 고객 언어로 풀어내어 후속 영업 파이프라인으로 연결',
      '반복되는 이메일 발송 업무를 구글 시트 기반 <strong>자동 메일 발송 시스템으로 직접 구현</strong><br>— 수작업 제거로 팀 발송 효율 개선 및 담당자가 고객 소통에 집중할 수 있는 환경 조성',
    ],
    kpis: [],
  },
  {
    ...
    bullets: [
      '일 20곳+ 콜드콜 아웃바운드 집행<br>— <strong>거절 사유를 매일 기록·분석</strong>하여 콜 스크립트를 지속 개선, 미팅 전환율 월평균 <strong>10%</strong> 달성 (업계 평균 대비 고전환)',
      '나라비전·게임듀오 등 <strong>40여 개 기업 파트너십 성사</strong><br>— 고용노동부 지정 KPI 목표(20개사) 대비 <strong>달성률 200%</strong>, 교육 과정 수강 기업 수 기준 팀 내 최다 실적',
      '강남취창업허브 오프라인 세미나 기획·운영 총괄<br>— 잠재 고객을 직접 한 자리에 모아 온·오프라인 <strong>잠재고객 발굴 채널을 다양화</strong>하고 파이프라인 확대에 기여',
    ],
    kpis: [
  ...
  },
  {
    ...
    bullets: [
      '일 40곳 인바운드 콜 대응 — 기업 규모·업종별 <strong>맞춤 구독 패키지</strong>를 즉석 제안하여<br>월 전환율 <strong>15%</strong> (최대 <strong>40%</strong>) 달성, <strong>팀 평균(5%) 대비 3배</strong>',
      '고객 불만의 근본 원인이 원두별 추출 세팅 편차임을 데이터로 확인<br>→ 5가지 원두별 최적 세팅값 매뉴얼 직접 제작 + 현장 교육 실시<br>→ <strong>설치 시간 단축·초기 이탈률 감소</strong>, 팀 전체 품질 표준화',
    ],
    kpis: [
